...
background_picture = image.load("Background_game.png")
win_Background =transform.scale(image.load("Win_Background.png"),(window_width, window_height))
Game_over_Background = transform.scale(image.load("You_Died.png"),(window_width, window_height))
...
class RedButton(GameSprite):
    def __init__(self, picture, w, h, x, y):
        ...
        self.pressing_frames = [transform.scale(image.load(f"Red_Button_{i}.png"), (w,h)) for i in [1,2]]
...
class LaserGate(GameSprite):
    def __init__(self, picture, w, h, x, y,):
        super().__init__(picture, w, h, x, y)
        self.open_frames = [transform.scale(image.load(f"Red_Laser_Gate_{i}.png"), (w,h)) for i in [1, 2, 3, 4]]    
...
class ShootingEnemies_y(Hazard):
    ...
    def fire_vertical(self):
        ...
        Canon_Bullet = VerticalBullet("Canon_Bullet.png",CANON_BULLET_WIDTH, CANON_BULLET_HEIGHT, spawn_x, spawn_y, speed_y, self.damage)
...
class DragonShooter(DamageSprite):
    def __init__(self, picture, w, h, x, y, health, damage):
        ...
        self.frames_right = [transform.scale(image.load(f"Red_Dragon_Right_{i}.png"), (w, h) ) for i in range(1,4)]
...
class GreatDragon(DamageSprite):
    def __init__(self, w, h, x, y, health, damage):
        super().__init__("Dragon_Idle/001.png", w, h, x, y, health, damage)
        self.has_death_animation = True  # GreatDragon has a death animation
        self.states = {
            "idle": self.load_animation("Dragon_Idle", 161),
            "battle_idle": self.load_animation("Dragon_Idle_Battle", 140),
            "death": self.load_animation("Dragon_Dies", 301),
            "attack_melee": self.load_animation("Dragon_Attack_1", 161),
            "attack_range": self.load_animation("Dragon_Attack_2", 202)
        }
    ...
    def GreatDragon_fireball(self):
        Great_fire_ball = GreatDragonFireball("Great_Dragon_Fireball.png", 50, 50, 
...
class Player(DamageSprite):
    def __init__(self, w, h, x, y, x_speed, y_speed, health):
        super().__init__(f"1.1_Archer.Hero.Move.right.png",w,h,x,y, health,  0)
        ...
        self.frames_right = [transform.scale(image.load(f"1.{i}_Archer.Hero.Move.right.png"),(w,h)) for i in range(1, 11)]
        self.frames_left = [transform.scale(image.load(f"1.{i}_Archer.Hero.Move.left.png"),(w,h)) for i in range(1, 11)]  
        self.is_shooting = False
        self.shoot_frames_right = [transform.scale(image.load(f"1.{i}_Archer.Hero.Aiming.right.png"), (w,h)) for i in range(1,11)]
        self.shoot_frames_left = [transform.scale(image.load(f"1.{i}_Archer.Hero.Aiming.left.png"), (w,h)) for i in range(1,11)]
        self.shoot_frame_index = 0
        self.shot_frame_counter= 0
        self.shoot_frame_animation_speed = 5
        self.arrow_fire_permission = True
        self.death_frames_right = [transform.scale(image.load(f"1.{i}_Archer.Hero.Died.right.png"), (w,h)) for i in range(1,11)]
        self.death_frames_left = [transform.scale(image.load(f"1.{i}_Archer.Hero.Died.left.png"), (w,h)) for i in range(1,11)]
        self.is_dead =False 
        ...
        if self.is_shooting :
            ...
            if self.shoot_frame_counter >= self.shoot_frame_animation_speed:
                ...
                if self.shoot_frame_index == 7 and  self.arrow_fire_permission == True :
                    ...
                    if self.direct == "right":
                        img = "Simple_Arrow_Right.png"
                        start_x = self.rect.right
                        speed = 5
                    else:
                        img = "Simple_Arrow_Left.png"
...
class Enemy_x(DamageSprite):        
    def __init__(self, w, h, x, y, speed, health, damage):
        super().__init__( f"11.1_Ork.Walk.right.png",w, h, x, y, health, damage)
        self.speed = speed
        self.direction ="left"
        self.frame_index = 0
        self.frame_counter = 0 
        self.animation_speed = 5
        self.frames_right = [transform.scale(image.load(f"11.{i}_Ork.Walk.right.png") ,(w,h)) for i in range (1,8)]
        self.frames_left = [transform.scale(image.load(f"11.{i}_Ork.Walk.left.png") ,(w,h)) for i in range (1,8)]
        self.is_dead = False
...
wall_1 = GameSprite('desertwall.jpg', 40, 660, 0, 0) # wall 1
wall_2 = GameSprite('desertwall.jpg', 1350, 40, 0, 0) # wall 2
wall_3 = GameSprite('desertwall.jpg', 1350, 40, 0, 660) # wall 3
wall_4 = GameSprite('desertwall.jpg', 40, 580, 1310, 0) # wall 4
#Mase's walls                         w   h   x     y 
wall_5 = GameSprite('desertwall.jpg', 30, 120, 90, 90) # wall 5
wall_6 = GameSprite('desertwall.jpg', 25, 7, 92.5, 210) # wall 6
wall_7 = GameSprite('desertwall.jpg', 15, 15, 97.5, 217) # wall 7
wall_8 = GameSprite('desertwall.jpg', 30, 10, 90, 232) # wall 8
wall_9 = GameSprite('desertwall.jpg', 30, 10, 90, 292)  # wall 9
wall_10 = GameSprite('desertwall.jpg', 15, 15, 97.5, 302)  # wall 10
wall_11 = GameSprite('desertwall.jpg', 25, 7, 92.5, 317)  # wall 11
wall_12 = GameSprite('desertwall.jpg', 30,  336, 90, 324) # wall 12
wall_13 = GameSprite('desertwall.jpg', 110, 30, 340, 250) # wall 13
wall_14 = GameSprite('desertwall.jpg', 120, 30, 120, 90) # wall 14
wall_15 = GameSprite('desertwall.jpg', 25, 490, 165, 120) # wall 15
wall_18 = GameSprite('desertwall.jpg', 30, 410, 340, 280) # wall 18
wall_19 = GameSprite('desertwall.jpg', 240, 30, 300, 90) # wall 19
wall_20 = GameSprite('desertwall.jpg', 30, 50, 510, 40) # wall 20
wall_21 = GameSprite('desertwall.jpg', 30,160, 490, 120) # wall 21
wall_22 = GameSprite('desertwall.jpg', 220, 30, 520, 250) # wall 22
#                                       w   h   x     y 
wall_23 = GameSprite('desertwall.jpg', 380, 30, 370, 330) # wall 23
wall_24 = GameSprite('desertwall.jpg', 30, 230, 820, 200) # wall 24
wall_25 = GameSprite('desertwall.jpg', 700, 30, 560, 170) # wall 25
wall_26 = GameSprite('desertwall.jpg', 30, 80, 590, 90) # wall 26
wall_27 = GameSprite('desertwall.jpg', 30, 80, 670, 40) # wall 27
wall_28 = GameSprite('desertwall.jpg', 30, 80, 750, 90) # wall 28
wall_29 = GameSprite('desertwall.jpg', 30, 80, 830, 40) # wall 29
wall_30 = GameSprite('desertwall.jpg', 30, 80, 910, 90) # wall 30
wall_31 = GameSprite('desertwall.jpg', 30, 80, 990, 40) # wall 31
wall_32 = GameSprite('desertwall.jpg', 30, 80, 1070, 90) # wall 32
wall_33 = GameSprite('desertwall.jpg', 30, 80, 1150, 40) # wall 33
wall_34 = GameSprite('desertwall.jpg', 30, 80, 1230, 90) # wall 34
wall_35 = GameSprite('desertwall.jpg', 350, 30, 960, 250) # wall 35
wall_36 = GameSprite('desertwall.jpg', 30, 410, 880, 250) # wall 36
wall_37 = GameSprite('desertwall.jpg', 30, 120, 960, 280) # wall 37
wall_38 = GameSprite('desertwall.jpg', 30, 200, 960, 460) # wall 38
wall_39 = GameSprite('desertwall.jpg', 30, 330, 1040, 330) # wall 39
wall_40 = GameSprite('desertwall.jpg', 190, 30, 1070, 330) # wall 40
wall_41 = GameSprite('desertwall.jpg', 180, 30, 1130, 420) # wall 41
wall_42 = GameSprite('desertwall.jpg', 30, 20, 720, 360) # wall 42
wall_43 = GameSprite('desertwall.jpg', 450, 30, 430, 430) # wall 43
wall_44 = GameSprite('desertwall.jpg', 30, 20, 590, 410) # wall 44
wall_45 = GameSprite('desertwall.jpg', 30, 20, 510, 360) # wall 45
wall_46 = GameSprite('desertwall.jpg', 30, 20, 430, 410) # wall 46
wall_47 = GameSprite('desertwall_triangle.png', 40, 15, 1310, 580) # wall 47
wall_48 = GameSprite('desertwall.jpg', 7, 35, 120, 150) # wall 48


                                   #   w   h   x     y
invisible_wall = GameSprite('Invisible_wall.png', 30, 200, 650, 460) #The which prevent the user to by pass the Great_Dragon, and Kill him immediately

GREAT_DRAGON_MELEE_DAMAGE_ZONE =  GameSprite('Invisible_wall.png', 510, 200, 370, 460)

Red_Button = RedButton("Red_button_1.png",40,40,835,620)

Great_Dragon = GreatDragon(300,255,575,460, health =35, damage= 50) #health = 35

Laser_Gate = LaserGate("Red_Laser_Gate_1.png",80,30,740,250)

Portal_Gate = GameSprite("Portal_1.1.png", 80,80,1265,580)

spikes_16 = Hazard('bone_shield_Right.png', 8,   30,  240,  90,        LETHAL_DAMAGE  )
spikes_17 = Hazard('bone_shield_Left.png',  8,   30,  290,  90,        LETHAL_DAMAGE  )
#                                          w     h    x       y     damage
canon_up = ShootingEnemies_y("Canon_Up.png",               40,  50,    50,  610,    LETHAL_DAMAGE,    direction= "up")
canon_down = ShootingEnemies_y("Canon_Down.png",           40,  50,   127,  145,    LETHAL_DAMAGE,    direction = "down" )
#                                                           w,   h,     x,   y,     damage_number      bullet_direction
player = Player( 40, 40, 45, 250,     0,          0,         100) 
...
Red_Dragon = DragonShooter("Red_Dragon_Right_1.png", 113, 91, 300,  145,   health =9, damage=35)  #health= 9
#                                                    w    h    x     y   
